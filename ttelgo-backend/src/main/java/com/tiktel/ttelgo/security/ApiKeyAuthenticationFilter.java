...
public class ApiKeyAuthenticationFilter extends OncePerRequestFilter {
    ...
    // Admin endpoints use JWT authentication, not API keys
    private static final List<String> EXEMPT_PATHS = List.of(
        "/api/v1/auth/**",
        "/api/v1/admin/**",  // Admin endpoints use JWT auth, not API keys
        "/api/v1/health/**",
        "/api/v1/bundles/**",
        "/api/v1/faqs/**",
        "/api/v1/posts/**",
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, 
        ...
        log.debug("ApiKeyAuthenticationFilter processing: {}", requestPath);
        
        // Check if path is exempted
        boolean isExempted = EXEMPT_PATHS.stream()
            .anyMatch(pattern -> pathMatcher.match(pattern, requestPath));
        
        log.debug("Path {} exempted: {}", requestPath, isExempted);
        
        if (isExempted) {
            log.debug("Path {} is exempted, skipping API key check", requestPath);
        ...
        log.debug("API Key present for {}: {}", requestPath, apiKey != null);
        
        // For non-exempt paths, if no API key provided, let Spring Security handle it
        // Don't block here - let Spring Security's .authenticated() handle it
        if (apiKey == null || apiKey.isEmpty()) {
            log.debug("No API key provided for {}, passing to Spring Security", requestPath);
            filterChain.doFilter(request, response);
            return;
        }
